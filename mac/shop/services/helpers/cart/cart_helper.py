...
def store_session_cart(request, cart):
    request.session[CART_SESSION_KEY] = normalize_cart(cart)
...
def cart_api(request):  
    ...
    cart = data.get("cart", [])
    store_session_cart(request, cart)
    stored_cart = session_cart(request)
...
def get_cart_data(request):
    ...
    return {
        ...
        "count": sum(item["qty"] for item in stored_cart),
        "total_price": sum(
            (Decimal(str(item["price"])) * item["qty"] for item in stored_cart),
            Decimal("0.00"),
        ),
    }
