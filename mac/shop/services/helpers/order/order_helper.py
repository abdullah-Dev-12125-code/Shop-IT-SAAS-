...
from ....models import Product, Order, OrderItem, OrderUpdate
...
def build_order_items(cart):
    normalized_cart, total_price = normalize_cart_items(cart)
...
def create_order(request):
    if request.method == 'POST':
        ...
        with transaction.atomic():
            order_items, server_total, error = build_order_items(cart)
            if error:
